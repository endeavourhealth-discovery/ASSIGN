...
MATCHONE(adrec,orgpost,ui)    ;matches one address
	...
	n adeploc,original,try,reformed,adpart,adloc,adtown
...
try1 D match(original,adflat,adbuild,adbno,adepth,adstreet,adeploc,adloc,adpost,adf2,adb2,adtown)
	...
	. d matched
	i adstreet?1"mc".e d
	. D match(original,adflat,adbuild,adbno,adepth,"mac"_$e(adstreet,3,50),adeploc,adloc,adpost,adf2,adb2,adtown)
	I $D(^TUPRN($J,"MATCHED")) d  q
	. d matched	
	I '$d(^TUPRN($J,"MATCHED")) D  Q
	. d nomatch
...
match(original,adflat,adbuild,adbno,adepth,adstreet,adeploc,adloc,adpost,adf2,adb2,adtown) ;
	;Match algorithms
	K ^TPROBABLE($j),^TPOSSIBLE($J),^TFIRSTFLAT($J),^THASFLAT($J)
...
300	;	
	...
	. s ALG="300-"
	. s matchrec=",Se"
	. s matches=$$match7(adpost,adstreet,adbno,adbuild,adflat,adloc,adtown)
...
350 ;near post code Matches with flat and building split out from street
	...
	. S ALG="350-"
	. s matches=$$match28(adpost,adstreet,adbno,adbuild,adflat,adloc,adtown)
	I $D(^TUPRN($J,"MATCHED")) Q
	;	
...
310	;	
	I adbuild'="" d
	 . s ALG="310-"
	. s matches=$$match7(adpost,adbuild,adflat,adstreet,adbno,adloc,adtown)
	;	
...
2201 ;Concatenate number and flat
	...
	. s ALG="2201-"
	. s matchrec=",Se"
	. s matches=$$match7(adpost,adstreet,adbno_adflat,adbuild,"",adloc,adtown)
	I $D(^TUPRN($J,"MATCHED")) Q	
	;	
...
2400 ;
		s ALG="2400-"
	s matchrec="Pe"
	s matches=$$match11(adpost,adstreet,adbno,adbuild,adflat,adloc,adepth,adeploc,adtown)
	I $D(^TUPRN($J,"MATCHED")) Q
	;	
...
3200 ;Post code matchS
	...
	I $D(^TUPRN($J,"MATCHED")) Q
	;
3250 ;Flat suffix in number and suffix is equivalent to building
	S ALG="3250-"
	s matches=$$match61b(adpost,adstreet,adbno,adbuild,adflat,adloc)
...
5200	;
	s ALG="5200-"
	i adpost=""!'$d(^UPRNX("X5",adpost)) s matches=$$match79^UPRNC(adstreet,adbno,adbuild,adflat,adepth,adeploc,adloc,adtown)
	I $D(^TUPRN($J,"MATCHED")) Q
5300	;	
	S ALG="5300-"
	i adpost=""!'$d(^UPRNX("X5",adpost)) s matches=$$match80^UPRNC(adstreet,adbno,adbuild,adflat,adepth,adeploc,adloc,adtown,adpost)
	I $D(^TUPRN($J,"MATCHED")) q
5400	;
	;Scottish format level / flat is avtually flat / number,area post code
	;Scottish stree number and flat 33-2f1, area post code
	;building may be street
	S ALG="5400-"
	s matches=$$match83^UPRNC(adstreet,adbno,adbuild,adflat,adepth,adeploc,adloc,adtown,adpost)
	i $D(^TUPRN($J,"MATCHED")) q
5500	;
	;Area post code match, organisation is building and otherwise match
	S ALG="5500-"
	s matches=$$match81^UPRNC(adstreet,adbno,adbuild,adflat,adepth,adeploc,adloc,adtown,adpost) 	;
	Q
	;	
	;	
	;
...
match29a(tpost,tstreet,tbno,tbuild,tflat)        ;
	...
	. . . . D GETADR^UPRNU(uprn,table,key,.flat,.build,.bno,.depth,.street,.deploc,.loc,.town,.lpost,.org)
	. . . . i bno=tbno,street'="",$$equiv^UPRNU(street,tstreet) d  q:matched
	. . . . . s $p(matchrec,",",2,5)="Se,Ne,Be,Fe"
	. . . . . s $P(ALG,"-",2)="match29aa"
	. . . . . s matched=$$setuprns("X5",post,street,tbno,tbuild,tflat)
	. . . . I buildroad,street=tbuild,bno=tflat,tbno="",flat="",build="" d  q
...
match6b(tpost,tstreet,tbno,tbuild,tflat)          ;
	...
	;	
	;	
match7(tpost,tstreet,tbno,tbuild,tflat,tloc,ttown)          ;
	...
	. s matches=$$match7b(post,tstreet,tbno,tbuild,tflat,tloc,ttown)
	i $G(^TUPRN($J,"MATCHED"))>1 d prefer	
	q $g(^TUPRN($J,"MATCHED"))
...
prefer ;
	...
	Q
	;	
match7b(post,tstreet,tbno,tbuild,tflat,tloc,ttown)         ;
	;Called from match7
	;Matches using X3
	...
	. . . s lprec=^UPRN("U",uprn,table,key)
	. . . D GETADR^UPRNU(uprn,table,key,.flat,.build,.bno,.depth,.street,.deploc,.loc,.town,.post,.org)
s	. . . S $p(matchrec,",",3)=$s(bno=tbno:"Ne",bno=""&(tbno'=""):"Nd",1:"Ni")
	. . . i build="",tbuild="" d  q
	. . . . s $p(matchrec,",",4)="Be"
	. . . . i flat="",tflat="" d  q
	. . . . . s $p(matchrec,",",5)="Fe"
	...
	. . . . . s matched=$$set(uprn,table,key)
	. . . s $p(matchrec,",",4)=""
	. . . i $$equiv^UPRNU(build,tstreet),tbuild="" d
	. . . . s $p(matchrec,",",4)="Bl"
	...
	. . . . s $P(ALG,"-",2)="match8e"
	. . . . s matched=$$set(uprn,table,key)
	. . . i tflat=bno,tbno=flat,street'="",tbuild="",tloc=loc,ttown=town d
	. . . . s $p(matchrec,",",2)="Ne"
	. . . . s $p(matchrec,",",2)="Si"
	. . . . s $p(matchrec,",",5)="Fe"
	. . . . s $p(matchrec,",",4)="B<S"
...
match6(tpost,tstreet,tbno,tbuild,tflat) 
	...
	. . i $$mno1(tbno,bno,.approx) d
	. . . I $D(^UPRNX("X5",tpost,tstreet,bno,tbuild,tflat)) d  Q
...
fnsplit(tbno,bno,tflat,flat) ;Number includes flat
	...
	q 0
match11(tpost,tstreet,tbno,tbuild,tflat,tloc,tdepth,tdeploc,ttown) 
...
match46(tpost,tstreet,tbno,tbuild,tflat) ;
	...
	. . . . . s matched=$$set(uprn,table,key)
	Q $G(^TUPRN($J,"MATCHED"))
match61b(tpost,tstreet,tbno,tbuild,tflat,tloc) ;
	;Long shot on post code , close on street
	I tbuild=""!(tstreet=""&(tloc="")) q ""
	s post=""
	...
	. . . . s street=$p(rec,"~",5),deploc=$p(rec,"~",6)
	. . . . S loc=$p(rec,"~",7),town=$p(rec,"~",8)
	. . . . i tflat'="",tbno'="",tstreet'="" d
	. . . . . i bno'=tbno q
	. . . . . i $$equiv^UPRNU(tstreet,street,1) d
	. . . . . . s $p(matchrec,",",2,5)="Sp,Ne,Be,Fe"
	. . . . . . s $P(ALG,"-",2)="match61b1"
	. . . . . . s matched=$$set(uprn,table,key)
	. . . . i tbuild'="",bno=tbno,tbuild=build,tstreet="",street="",tloc=loc d
	. . . . . s $p(matchrec,",",2,5)="Se,Ne,Be,Fe"
	. . . . . s $p(ALG,"-",2)="match61b2"
	. . . . . s matched=$$set(uprn,table,key)
	Q $G(^TUPRN($J,"MATCHED"))
...
match27(tpost,tstreet,tbno,tbuild,tflat) 
	...
	;	
	;	
match28(tpost,tstreet,tbno,tbuild,tflat,tloc,ttown) 
	...
	. . s matches=$$match7(tpost,t2,"",t1,tflat,tloc,ttown)
	q $G(^TUPRN($J,"MATCHED"))
	;	
...
match62(tpost,tstreet,tbno,tbuild,tflat,tloc,tdeploc) 
	...
	. . s matched=$$setuprns("X5",post,tstreet,tbno,tbuild,tflat)
	i matched q 1
	i tstreet="",tbno="",tbuild'="",tflat'="" d
	. s post="",matched=0
	. for  s post=$O(^UPRNX("X3",ZONE,tbuild,tflat,post)) q:post=""  d  q:matched
	. . s near=$$justarea(post,adpost)
	. . q:near=""
	. . s $p(matchrec,",",1)=near
	. . i $D(^UPRNX("X5",post,tbuild,tflat,"","")) d
	. . . s $p(matchrec,",",2,5)="Se,Ne,Be,Fe"
	. . . s $P(ALG,"-",2)="match62"
	. . . s matched=$$setuprns("X5",post,tbuild,tflat,"","") 
	Q $G(^TUPRN($J,"MATCHED"))
...
mflat(tpost,tstreet,tbno,tbuild,tflat,flat,approx)         ;
	...
	s scotflat=""
	;	
	I tflat?1"0"1"/"1n.n d  i matched q 1
	. s scotflat="/"_$p(tflat,"/",2)
...
mflat1(tflat,flat,approx) ;Matches two flats
	...
	. I tflat?1"g"1n.n d
	. . i flat?1"/"1n.n d
	. . . I $p(tflat,"g",2)=$p(flat,"/",2) s approx="p",matched=1
	i matched q 1
	i tflat["-" d
...
set(uprn,table,key,dualuse) ;
	...
	. S ^TUPRN($J,"MATCHED",uprn,table,key,"A")=ALG
	. S ^TUPRN($J,"MATCHED",uprn,table,key,"address")=^UPRN("U",uprn,table,key,"O")
	e  d  q 0
