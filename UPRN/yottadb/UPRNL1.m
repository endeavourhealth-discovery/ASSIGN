UPRNL1 ;Date Time Uitilities [ 06/09/2019  11:29 AM ]
	;**ESCL 5402727219B PLEASE DO NOT REMOVE.**
	; (c) copyright 1997. Egton Medical Information Systems 
	; Ltd. All rights reserved.;
	;downloaded 5.1 beta by RMI 1:33 pm  15.3.2012
	;downloaded beta by AW 3:40 pm  18.2.98
	;downloaded beta by AW 3:37 pm  20.3.97
	;downloaded beta by AW 2:08 pm  20.3.97
	;downloaded beta by LS 3:57 pm  26.9.97
	; &&& Cache Conversion 15.3.2012 : by RETURN
...
20 S ZX=ZX>21914+ZX
	...
	S ZX=%Y_"-"_$TR($J(%M,2)," ",0)_"-"_$TR($J(%D,2)," ",0)
	Q ZX
	;
...
DF(D1,D2)          ;difference between 2 dates
	...
	S DIF=D2-D1
	Q DIF
DIFDISP(start,end)  ;Returns a duisplay of time difference
	n total,days,hours,minutes,seconds
	s total=$$HDIFS(start,end)
	s days=total\(3600*24)
	s hours=total-(days*3600*24)\3600
	s minutes=total-((days*3600*24)+(hours*3600))\60
	s seconds=total-((days*3600*24)+(hours*3600)+(minutes*60))
	q days_" days "_hours_" hours "_minutes_" minutes "_seconds_" seconds"
HDIFS(start,end) ;Difference betweeen 2 $h times in seconds
		n seconds,days
		s seconds=$p(end,",",2)-$p(start,",",2)
		s days=($p(end,",",1)-$p(start,",",1))*(3600*24)
		q (days+seconds)
	;
	;	
